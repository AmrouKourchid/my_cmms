...
app.post('/createWorkRequest', authenticateToken, upload.array('images', 10), (req, res) => {
  const { client_id, site, asset_id, date_of_fault, description } = req.body;
  const images = req.files.map(file => file.buffer.toString('base64'));

  const query = 'INSERT INTO work_request (client_id, site, asset_id, date_of_fault, description, images) VALUES (?, ?, ?, ?, ?, ?)';

  db.query(query, [parseInt(client_id), site, asset_id, date_of_fault, description, JSON.stringify(images)], (err, results) => {
    if (err) {
