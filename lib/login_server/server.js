...
app.post('/createReport', authenticateToken, upload.array('pictures', 10), (req, res) => {
  ...
  db.query(query, [parseInt(worker_id), parseInt(work_order_id), Question1, Question2, Question3, Question4, Question5, Question6, JSON.stringify(pictures)], (err, results) => {
    ...
    // Insert materials into report_material table
    const materialQueries = materials.map(materialId => {
      return new Promise((resolve, reject) => {
        const materialQuery = 'INSERT INTO report_material (report_id, material_id) VALUES (?, ?)';
        db.query(materialQuery, [reportId, parseInt(materialId)], (err, results) => {
          if (err) {
            reject(err);
          } else {
            resolve(results);
    ...
    Promise.all(materialQueries)
      ...
      .catch(err => {
        ...
        res.status(500).send('Error inserting materials');
      });
  });
});
...
app.get('/reportByWorkOrderId/:workOrderId', authenticateToken, (req, res) => {
  ...
  const query = `
    SELECT r.question1, r.question2, r.question3, r.question4, r.question5, r.question6, w.name as worker_name, r.pictures,
           m.name as material_name, m.image as material_image, m.cost as material_cost
    FROM reports r
    JOIN worker w ON r.worker_id = w.id
    LEFT JOIN report_material rm ON r.id = rm.report_id
  ...
  db.query(query, [workOrderId], (err, results) => {
    ...
    const report = {
      question1: results[0].question1,
      question2: results[0].question2,
      question3: results[0].question3,
      question4: results[0].question4,
      question5: results[0].question5,
      question6: results[0].question6,
      worker_name: results[0].worker_name,
      pictures: results[0].pictures ? JSON.parse(results[0].pictures) : [],
      materials: results.map(row => ({
        name: row.material_name,
        image: row.material_image ? row.material_image.toString('base64') : null,
        cost: row.material_cost
      })).filter(material => material.name) // Filter out null materials
    };
...
// API to display materials for a specific worker
app.get('/displayMaterials', authenticateToken, (req, res) => {
  const workerId = req.user.id;
  const query = 'SELECT id, name, cost, image FROM material WHERE worker_id = ?';
  db.query(query, [workerId], (err, results) => {
    if (err) {
...
app.post('/createMaterial', authenticateToken, upload.single('image'), (req, res) => {
  const { name, cost } = req.body;
  const image = req.file ? req.file.buffer : null;
  const workerId = req.user.id;

  const query = 'INSERT INTO material (worker_id, name, cost, image) VALUES (?, ?, ?, ?)';
  db.query(query, [workerId, name, cost, image], (err, results) => {
    if (err) {
...
app.delete('/deleteMaterial/:id', authenticateToken, (req, res) => {
  const { id } = req.params;

  const deleteQuery = 'DELETE FROM material WHERE id = ?';
  db.query(deleteQuery, [id], (err, results) => {
    if (err) {
